 ...
 */
define('WFDOWNLOADS_REQUEST_NOTRIM', 1);
define('WFDOWNLOADS_REQUEST_ALLOWRAW', 2);
define('WFDOWNLOADS_REQUEST_ALLOWHTML', 4);
...
class WfdownloadsRequest
{
    ...
    static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
    {
        ...
        if (isset($input[$name]) && $input[$name] !== null) {
            ...
            $var = WfdownloadsRequest::_cleanVar($input[$name], $mask, $type);
            // Handle magic quotes compatability
            if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
                $var = WfdownloadsRequest::_stripSlashesRecursive($var);
            }
        } elseif ($default !== null) {
            // Clean the default value
            $var = WfdownloadsRequest::_cleanVar($default, $mask, $type);
        } else {
            $var = $default;
        }
