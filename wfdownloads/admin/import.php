...
function import_wmpdownloads_to_wfdownloads()
{
    ...
    $xoopsDB->query(
        ...
        . " (`old_cid`, `old_pid`, `title`, `imgurl`, `summary`) SELECT `cid`, `pid`, `title`, `imgurl`, '' FROM " . $source['cat']
    );
    echo "Imported " . $xoopsDB->getAffectedRows() . " categories into " . $destination['cat'] . "<br />";
    //Import data into downloads table
    $xoopsDB->query(
        "INSERT INTO " . $destination['downloads'] . " (`cid`, `old_lid`, `old_cid`, `title`, `url`, `homepage`, `version`, `size`, `platform`, `screenshot`, `submitter`, `status`, `published`, `hits`, `rating`, `votes`, `comments`, `features`, `requirements`, `dhistory`, `summary`, `description`)
                    SELECT 0,`lid`, `cid`, `title`, `url`, `homepage`, `version`, `size`, `platform`, `logourl`, `submitter`, `status`, `date`, `hits`, `rating`, `votes`, `comments`, '', '','','', '' FROM "
        . $source['downloads']
    );
    echo "Imported " . $xoopsDB->getAffectedRows() . " downloads into " . $destination['downloads'] . "<br />";
    //Import data into brokens table
    $xoopsDB->query(
        "INSERT INTO " . $destination['broken'] . " (`lid`, `sender`, `ip`)
    ...
    if ($max_requestid) {
        $xoopsDB->query(
            "UPDATE " . $destination['mod'] . " m, " . $destination['cat'] . " c SET m.cid=c.cid WHERE m.requestid > " . $max_requestid
            . " AND c.old_cid=m.cid"
        );
        //Update lid values in mod table
        $xoopsDB->query(
            "UPDATE " . $destination['mod'] . " m, " . $destination['downloads'] . " d SET m.lid=d.lid WHERE m.requestid > " . $max_requestid
            . " AND m.lid=d.old_lid"
        );
    }
    if ($max_ratingid) {
        //Update lid values in votedata table
        $xoopsDB->query(
            "UPDATE " . $destination['votes'] . " v, " . $destination['downloads'] . " d SET v.lid=d.lid WHERE v.ratingid > " . $max_ratingid
            . " AND v.lid=d.old_lid"
        );
    }
    if ($max_reportid) {
        ...
        $xoopsDB->query(
            "UPDATE " . $destination['broken'] . " b, " . $destination['downloads'] . " d SET b.lid=d.lid WHERE b.reportid > " . $max_reportid
            . " AND b.lid=d.old_lid"
        );
    }
    //Update description
