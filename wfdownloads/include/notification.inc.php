...
function wfdownloads_notify_iteminfo($category, $item_id)
{
    ...
    if ($category == 'category') {
        ...
        $sql = "SELECT title FROM " . $xoopsDB->prefix('wfdownloads_cat') . " WHERE cid = '" . (int) $item_id . "'";
    ...
    if ($category == 'file') {
        ...
        $sql = "SELECT cid,title FROM " . $xoopsDB->prefix('wfdownloads_downloads') . " WHERE lid = '" . (int) $item_id . "'";
        $result = $xoopsDB->query($sql); // TODO: error check
        $result_array = $xoopsDB->fetchArray($result);
        $item['name'] = $result_array['title'];
